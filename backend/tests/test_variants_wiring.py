"""The identity-variant wiring provider builds a request-scoped service over the seam.

Confirms ``build_variant_service_provider`` reads the write-event publisher off
...
def test_provider_binds_the_session_and_the_app_publisher() -> None:
    ...
    provider = build_variant_service_provider()
    service = provider(request, session)

    assert isinstance(service, IdentityVariantService)
