"""In-memory test doubles for the accounts domain.

The service layer is tested sociably: the real password hasher (at a cheap cost),
the real token codec, and this in-memory repository substituted at the repository
interface (the only true external boundary, Postgres). The fake enforces the same
unique-email constraint as the real ``users`` table and assigns the server-minted
id the identity column would, so the duplicate-conflict and session paths are
exercised without a database.
"""

from __future__ import annotations

from datetime import datetime, timedelta
from typing import TYPE_CHECKING

from pydantic import SecretStr
from sqlalchemy.exc import IntegrityError

from floresu.accounts.config import SessionConfig
from floresu.accounts.injection import Clock, utcnow
from floresu.accounts.passwords import BcryptPasswordHasher
from floresu.accounts.tokens import RefreshClaims, SessionTokenCodec

if TYPE_CHECKING:
    from floresu.accounts.models import User

# Cheap bcrypt cost for tests: the real hashing + verification path, fast. Cost 12
# is asserted directly in test_accounts_passwords.
TEST_BCRYPT_COST = 4
# >= 32 bytes so HS256 signing raises no InsecureKeyLengthWarning under pyjwt.
TEST_SESSION_SECRET = "unit-test-session-secret-0123456789"


class _UniqueViolationError(Exception):
    """Mimics an asyncpg unique-violation carried on ``IntegrityError.orig``."""

    sqlstate = "23505"


class InMemoryAccountRepository:
    """A dict-backed :class:`AccountRepository` with real uniqueness + identity."""

    def __init__(self) -> None:
        self._by_id: dict[int, User] = {}
        self._revoked: set[str] = set()
        self._next_id = 1
        self.commits = 0
        self.rollbacks = 0

    async def get_by_email(self, email: str) -> User | None:
        return next((u for u in self._by_id.values() if u.email == email), None)

    async def get_by_id(self, user_id: str) -> User | None:
        try:
            return self._by_id.get(int(user_id))
        except ValueError:
            return None

    async def mark_onboarding_complete(self, user_id: str) -> User | None:
        user = await self.get_by_id(user_id)
        if user is None:
            return None
        user.has_completed_onboarding = True
        return user

    async def add_user(self, user: User) -> None:
        if any(existing.email == user.email for existing in self._by_id.values()):
            raise IntegrityError("INSERT INTO users", {}, _UniqueViolationError())
        # The real identity column assigns the id on flush; mirror that so the
        # service can mint a session for a server-minted id.
        user.id = self._next_id
        self._next_id += 1
        self._by_id[user.id] = user

    async def is_session_revoked(self, sid: str) -> bool:
        return sid in self._revoked

    async def revoke_session(self, claims: RefreshClaims) -> None:
        self._revoked.add(claims.sid)

    async def commit(self) -> None:
        self.commits += 1

    async def rollback(self) -> None:
        self.rollbacks += 1


def build_test_codec(
    *,
    access_ttl: timedelta = timedelta(minutes=15),
    refresh_ttl: timedelta = timedelta(days=14),
    clock: Clock = utcnow,
) -> SessionTokenCodec:
    """A codec with the test secret and overridable TTLs/clock (for expiry tests)."""
    return SessionTokenCodec(
        SessionConfig(
            secret=SecretStr(TEST_SESSION_SECRET), access_ttl=access_ttl, refresh_ttl=refresh_ttl
        ),
        clock=clock,
    )


class MutableClock:
    """A pinned, advanceable clock for expiry tests (no ``sleep``/negative TTL)."""

    def __init__(self, now: datetime) -> None:
        self._now = now

    def __call__(self) -> datetime:
        return self._now

    def advance(self, delta: timedelta) -> None:
        self._now += delta


def build_test_hasher() -> BcryptPasswordHasher:
    """The real bcrypt hasher at a cheap cost for fast tests."""
    return BcryptPasswordHasher(cost=TEST_BCRYPT_COST)
