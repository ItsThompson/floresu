...
context -> decision -> ``/oauth/token`` PKCE -> refresh -> revoke) and asserts the
wire contracts: OAuth ``error`` JSON on the protocol endpoints, problem+json on
the SPA endpoints, the Site-URL gotcha (metadata/redirect URLs come from pinned
...
from floresu.core.identity import SESSION_COOKIE_NAME, StripInboundIdentityMiddleware
...
def _register(client: TestClient) -> str:
    response = client.post(
        "/oauth/register", json={"redirect_uris": [_REDIRECT], "client_name": "Claude"}
    )
...
def test_as_metadata_urls_come_from_config_not_request_host(make_settings: MakeSettings) -> None:
    ...
    # The request arrives at "backend:8000" (the tunnel origin), but the issuer and
    # endpoints must be the pinned public URLs.
...
def test_jwks_endpoint_publishes_public_keys(make_settings: MakeSettings) -> None:
    fx = _build_client(make_settings)
    response = fx.client.get("/oauth/jwks")
    assert response.status_code == 200
...
def test_register_mints_a_client_id(make_settings: MakeSettings) -> None:
    fx = _build_client(make_settings)
    assert _register(fx.client)


def test_register_bad_redirect_is_oauth_error_json(make_settings: MakeSettings) -> None:
    fx = _build_client(make_settings)
    response = fx.client.post("/oauth/register", json={"redirect_uris": ["http://evil.example/cb"]})
    assert response.status_code == 400
    assert response.json()["error"] == "invalid_client_metadata"


def test_register_empty_redirect_uris_is_problem_json(make_settings: MakeSettings) -> None:
    # The schema's min_length=1 rejects an empty list at the DCR boundary
    # (RFC 9457 problem+json) before the service validation loop.
    fx = _build_client(make_settings)
    response = fx.client.post("/oauth/register", json={"redirect_uris": []})
...
def test_context_reports_authenticated_flag(make_settings: MakeSettings) -> None:
    ...
    anon = fx.client.get("/oauth/authorize/context", params={"auth_request_id": auth_request_id})
    assert anon.status_code == 200
    assert anon.json()["authenticated"] is False
    assert anon.json()["client_name"] == "Claude"
...
def test_decision_without_a_session_is_401_problem_json(make_settings: MakeSettings) -> None:
    ...
    response = fx.client.post(
        "/oauth/authorize/decision", json={"auth_request_id": "whatever", "approve": True}
    )
...
def test_missing_context_is_404_problem_json(make_settings: MakeSettings) -> None:
    fx = _build_client(make_settings)
    response = fx.client.get("/oauth/authorize/context", params={"auth_request_id": "nope"})
...
def test_full_pkce_flow_issues_an_audience_bound_token(make_settings: MakeSettings) -> None:
    ...
    assert body["scope"] == "floresu:full"
...
def test_me_clients_lists_and_revokes_own_clients(make_settings: MakeSettings) -> None:
    ...
    entries = listed.json()
    assert [c["client_id"] for c in entries] == [client_id]
    assert "connected_at" in entries[0] and "last_active_at" in entries[0]

    revoked = fx.client.delete(
        f"/me/clients/{client_id}", cookies={SESSION_COOKIE_NAME: _SESSION_COOKIE}
    )
    assert revoked.status_code == 204
    empty = fx.client.get("/me/clients", cookies={SESSION_COOKIE_NAME: _SESSION_COOKIE})
    assert empty.json() == []
