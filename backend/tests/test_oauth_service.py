"""Sociable service tests: the full AS flow in-process.
...
downgrade/mismatch/replay/expiry failure paths and the connected-client surface.
"""
...
from floresu.core.observability import FLORESU_REGISTRY
from floresu.oauth.authorization import AuthorizationService
from floresu.oauth.config import SCOPE_FULL
from floresu.oauth.errors import OAuthError, OAuthErrorCode
...
async def _register(auth: AuthorizationService, *, scope: str | None = None) -> str:
    response = await auth.register_client(
        ClientRegistrationRequest(redirect_uris=[_REDIRECT], client_name="Claude", scope=scope)
    )
...
async def test_register_mints_a_public_pkce_client_with_defaults() -> None:
    h = _harness()
    ...
    assert response.scope == SCOPE_FULL
    assert await h.repo.get_client(response.client_id) is not None


async def test_register_defaults_the_client_name_when_absent() -> None:
    h = _harness()
    response = await h.auth.register_client(ClientRegistrationRequest(redirect_uris=[_REDIRECT]))
    assert response.client_name == "Unnamed agent"
...
async def test_register_rejects_dangerous_or_non_allowlisted_redirects(redirect_uri: str) -> None:
    # Allowlist: only https-with-a-host or loopback http may register. Everything
    # else is rejected at DCR so it can never become a consent navigation target.
    h = _harness()
...
async def test_authorize_rejects_missing_pkce_challenge() -> None:
    h = _harness()
    client_id = await _register(h.auth)
    with pytest.raises(OAuthError) as exc:
        await _park(h.auth, client_id, "", code_challenge_method="S256")
    assert exc.value.error == OAuthErrorCode.INVALID_REQUEST
...
async def test_get_context_returns_client_name_scope_and_auth_flag() -> None:
    h = _harness()
    client_id = await _register(h.auth)
    _verifier, challenge = make_pkce_pair()
    request_id = await _park(h.auth, client_id, challenge)
    context = await h.auth.get_context(request_id, authenticated=False)
    assert context.client_name == "Claude"
    assert context.scopes == [SCOPE_FULL]
...
async def test_approve_mints_a_code_and_records_the_grant() -> None:
    h = _harness()
    client_id = await _register(h.auth)
    _verifier, challenge = make_pkce_pair()
    request_id = await _park(h.auth, client_id, challenge)
...
async def test_code_exchange_returns_audience_bound_access_and_refresh() -> None:
    ...
    assert result.scope == SCOPE_FULL
...
async def test_code_exchange_rejects_a_revoked_grant() -> None:
    ...
    # the user revoked the client cannot mint a fresh refresh token.
    h = _harness()
    client_id = await _register(h.auth)
    verifier, challenge = make_pkce_pair()
    code = await _authorize_to_code(h, client_id, challenge)
...
async def test_refresh_fails_closed_when_the_client_is_gone() -> None:
    # Defense in depth: a refresh whose client row was dropped fails closed rather
    # than minting a fresh pair for a client that no longer exists.
    h = _harness()
    client_id = await _register(h.auth)
...
async def test_list_connected_clients_reports_connected_and_last_active_times() -> None:
    clock = MutableClock(datetime(2024, 1, 1, tzinfo=UTC))
    h = _harness(clock=clock)
    client_id = await _register(h.auth)
    first = await _issue_tokens(h, client_id)
    connected_at = clock()
    # A later refresh advances last-active but not the connected time.
    ...
    assert entry.client_name == "Claude"
    assert entry.scopes == [SCOPE_FULL]
    assert entry.connected_at == connected_at
    assert entry.last_active_at == connected_at + timedelta(hours=3)


async def test_revoke_connected_client_kills_refresh_and_requires_fresh_consent() -> None:
    h = _harness()
    client_id = await _register(h.auth)
    first = await _issue_tokens(h, client_id)
    await h.tokens.revoke_connected_client(_USER, client_id)
    ...
    with pytest.raises(NotFound):
        await h.tokens.revoke_connected_client(_USER, client_id)
    # Reconnecting requires a fresh consent + a new code (a new grant + tokens).
    reconnected = await _issue_tokens(h, client_id)
    assert h.codec.verify(reconnected.access_token) is not None
    assert [c.client_id for c in await h.tokens.list_connected_clients(_USER)] == [client_id]


async def test_reconsent_resets_the_connected_time() -> None:
    clock = MutableClock(datetime(2024, 1, 1, tzinfo=UTC))
    h = _harness(clock=clock)
    client_id = await _register(h.auth)
    ...
    await _issue_tokens(h, client_id)
    (entry,) = await h.tokens.list_connected_clients(_USER)
    assert entry.connected_at == datetime(2024, 1, 3, tzinfo=UTC)


async def test_list_connected_clients_skips_a_deleted_client() -> None:
    h = _harness()
    kept = await _register(h.auth)
...
def _issued_count(grant_type: str) -> float:
    value = FLORESU_REGISTRY.get_sample_value(
        "oauth_tokens_issued_total", {"grant_type": grant_type}
    )
    return value or 0.0
