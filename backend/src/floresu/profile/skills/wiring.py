...
def build_skill_service_provider() -> Callable[..., SkillService]:
    """A FastAPI dependency that builds a request-scoped :class:`SkillService`."""

    def provider(request: Request, session: AsyncSession = Depends(get_session)) -> SkillService:
        publisher: WriteEventPublisher = request.app.state.events
        return SkillService(session, SqlAlchemySkillRepository(session), publisher)
