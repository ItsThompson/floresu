...
``path`` is the matched route template (e.g. ``/resumes/{id}``) to bound label
...
concatenated with the shared :data:`~floresu.core.observability.FLORESU_REGISTRY`
...
def _expose_combined(app: FastAPI, http_registry: CollectorRegistry) -> None:
    ...
    async def metrics() -> Response:
        payload = generate_latest(http_registry) + generate_latest(FLORESU_REGISTRY)
