...
FastAPI dependency every repository depends on, plus the write-transaction
boundary the service layer wraps its mutates in (:func:`transaction`) and the two
query helpers it needs: :func:`is_unique_violation` (Postgres unique-constraint
breach -> a signal the service raises as ``Conflict``) and :func:`fetch_optional`
(no-rows fetch -> ``None``).

The pool is bounded and sized for a single backend instance (~5 concurrent users,
one VPS). Connecting is lazy, so building an app that wires the engine does not
require a reachable database; the ``/readyz`` check (:func:`db_readiness_check`)
is what surfaces connectivity.
"""
...
# ``get_session`` is a FastAPI dependency, not a decorated route handler. FastAPI
# resolves each parameter annotation at runtime to build the dependency, so
# ``Request`` must stay importable at runtime for FastAPI to recognize the special
# request-injection parameter instead of treating ``request`` as a query field.
...
# severed by a Postgres restart or idle timeout before they are handed to a
# request.
POOL_SIZE = 5
MAX_OVERFLOW = 5
...
# Perf-counter start times are stashed per connection so a nested/executemany
# statement pops its own start rather than a shared global.
_QUERY_START_KEY = "floresu_query_start"
...
async def get_session(request: Request) -> AsyncIterator[AsyncSession]:
    ...
    in tests. The session is yield-only: the service layer owns the commit/rollback
...
async def transaction(session: AsyncSession) -> AsyncIterator[None]:
    """Single-source the write invariant: commit on a clean exit, roll back and
    re-raise on any error.

    The caller performs an arbitrary mutate (add / update / delete) inside the
    ``async with`` block; this boundary commits it, or rolls back and re-raises if
