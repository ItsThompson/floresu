...
Thin handlers: each maps one request to one :class:`AccountService` call, writes
or clears the session cookies, and returns the authenticated user (never the
password hash). The ``FloresuError`` the service raises is rendered as RFC 9457
...
from floresu.accounts.schemas import AuthenticatedUser, LoginRequest, RegisterRequest
...
def create_accounts_router(
    ...
    async def register(
        ...
        session = await service.register(body.email, body.password)
