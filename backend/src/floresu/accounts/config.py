...
# (``floresu_session``); the refresh cookie is scoped to the /auth path so it is
# only sent to the endpoints that rotate or revoke it.
REFRESH_COOKIE_NAME = "floresu_refresh"
AUTH_PATH = "/auth"

# Short access token, long rotating refresh. Defaults live in the domain that
# owns them; overridable for tests.
...
class CookieConfig:
    ...
    ``Secure`` is on outside development (the tunnel terminates TLS at the edge);
    ``domain`` is the configured apex in prod so the SPA and API subdomains share
