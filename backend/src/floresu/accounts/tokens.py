...
access/refresh pair shares one opaque session id (``sid``); the ``sid`` is what
the blacklist revokes, so revoking a session invalidates the still-unexpired
access token immediately (not just future refreshes). This is why the identity
seam's ``SessionVerifier`` is async: verifying an access token includes a ``sid``
blacklist lookup.

The codec is pure crypto over the config: no I/O, no DB. The service layer owns
the blacklist and the rotation policy; this module only mints and verifies. The
``sub`` claim carries the user id as a string (the JWT registered-claim
convention); the caller resolves it back to the bigint ``users.id`` at the
repository boundary.
"""
