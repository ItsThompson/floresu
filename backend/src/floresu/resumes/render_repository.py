...
export records), and writes that object key back. Transaction ownership stays with
the service: :meth:`set_revision_pdf_key` issues the update, but the ``transaction``
boundary the service wraps it in is what commits.
"""
