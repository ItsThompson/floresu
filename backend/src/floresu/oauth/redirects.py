...
opens per run. The AS therefore permits loopback redirect URIs on **any port** as
long as scheme, host, and path match a registered loopback URI; non-loopback
...
def is_allowed_redirect(requested: str, registered: list[str]) -> bool:
    ...
    any registered loopback URI with the same scheme/host/path on a different port
    (RFC 8252), so an agent can bind a fresh ephemeral port each run.
    """
