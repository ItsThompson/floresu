...
(:meth:`floresu.oauth.token_exchange.TokenService.cleanup_stale_clients`) to a
...
from floresu.oauth.wiring import build_token_service
...
_log = get_logger("floresu-oauth-cleanup")
...
async def run_cleanup_loop(sweep: Sweep, *, interval: timedelta) -> None:
    ...
    while True:
        ...
        try:
            ...
            raise
        except Exception as exc:  # one bad sweep must not kill the reaper
