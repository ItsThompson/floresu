...
The OAuth handshake endpoints (``/oauth/token``, ``/oauth/register``,
``/oauth/revoke``, ``/oauth/authorize``) must return the OAuth-standard
``{"error", "error_description"}`` JSON that clients and the MCP SDK parse,
**not** the product's RFC 9457 problem+json. This module owns that contract and
its exception handler, wired only on the external app alongside the shared
``FloresuError`` handler. Product-shaped OAuth endpoints consumed by our own SPA
(``/oauth/authorize/context``, ``/oauth/authorize/decision``, ``/me/clients``)
keep raising ``FloresuError`` and render as problem+json.
"""
