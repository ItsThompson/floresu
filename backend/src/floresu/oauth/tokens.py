...
resource, so the resource server verifies them via the AS JWKS and rejects any
token whose ``aud`` is not the MCP resource. Refresh tokens are high-entropy
opaque strings stored only as a SHA-256 hash (secrets hashed at rest); rotation
is enforced by the service, which revokes the presented refresh and mints a fresh
...
class AccessTokenCodec:
    ...
    def verify(self, token: str) -> VerifiedAccessToken | None:
        ...
        The resource server owns request-time bearer validation; this mirror
        proves audience binding and expiry in the AS's own tests. Expiry is checked
        against the injected clock so a pinned clock governs the mint -> expire
        assertion.
        """
        try:
