...
The authorization-grant lifecycle up to code minting: a client registers
(RFC 7591), an ``/oauth/authorize`` request is validated and **parked**
...
:mod:`floresu.oauth.token_exchange`.

Protocol errors on the agent-facing ``/oauth/register`` and ``/oauth/authorize``
paths are ``OAuthError`` (RFC 6749 JSON); the SPA-facing context/decision paths
raise ``FloresuError`` (problem+json), since our own frontend consumes them.
"""
...
class AuthorizationService:
    ...
    def __init__(
        ...
        *,
        ...
        # globals; the defaults reproduce the ambient calls exactly.
        self._clock = clock
        # Opaque protocol ids (client_id, auth_request_id, code): url-safe tokens.
        self._new_id = new_id
        # Surrogate record keys threaded into the repo writes (grant id).
    ...
    async def start_authorization(self, params: AuthorizeParams) -> str:
        """Validate + park an ``/oauth/authorize`` request; return the SPA consent URL."""
    ...
    async def decide(self, *, auth_request_id: str, user_id: str, approve: bool) -> str:
        ...
        cannot re-approve. Approve records the connected-client grant; the returned
        loopback URL is what the SPA navigates the browser to.
        """
        request = await self._load_live_request(auth_request_id)
    ...
    def _reject_invalid_redirect_uris(self, uris: list[str]) -> None:
        for uri in uris:
            # Allowlist: a public client may register only an https redirect *with
            # a host* (exact-matched at /authorize) or a loopback http redirect
            # (RFC 8252, any port). Everything else -- javascript:, data:, file:,
            # custom schemes, non-loopback http, and a hostless "https:" URI -- is
            # rejected here so a dangerous or degenerate URI can never be parked or
            # handed back to the SPA as a navigation target. (An empty
            # redirect_uris list is rejected earlier by the schema's min_length=1.)
            parts = urlsplit(uri)
