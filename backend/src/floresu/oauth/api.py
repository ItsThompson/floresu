...
Thin handlers: each maps one request to one service call. The AS-metadata and JWKS
endpoints are static (built from pinned config + the signing key set at
router-creation time, never the request host). Protocol endpoints
(``/oauth/register``, ``/oauth/authorize``, ``/oauth/token``, ``/oauth/revoke``)
surface ``OAuthError`` as RFC 6749 JSON; the SPA-facing endpoints
(``/oauth/authorize/context``, ``/oauth/authorize/decision``, ``/me/clients``)
resolve the human session and render ``FloresuError`` as problem+json.
