...
# scripts/deploy.sh
#
# Stateless Docker Context deploy for Floresu. The Compose CLI runs HERE (a CI
# runner or an operator's checkout); the engine runs on the VPS, reached through
# a pre-registered Docker Context (ssh://deploy@<ip>, name `floresu`). ALL config
...
#                           executing.
#   FLORESU_DOCKER_CONTEXT  Docker context name (default `floresu`); CI registers it.
#   FLORESU_REMOTE_DIR      remote dir holding .deployed-sha (default /opt/floresu).
#
...
# checkout, and re-runs the deploy once with FLORESU_IMAGE_TAG=sha-<prev>, which
...
REMOTE_DIR="${FLORESU_REMOTE_DIR:-/opt/floresu}"
CONTEXT_NAME="${FLORESU_DOCKER_CONTEXT:-floresu}"

# Every compose invocation runs over the Docker Context and layers the tunnel
# overlay (owns ingress) and the deploy overlay (feeds backend/mcp/worker the
# committed .env.prod + app secrets client-side via env_file/environment:, since
# the box has no .env). The tunnels profile keeps cloudflared and alertmanager
# from starting outside a deploy.
...
REQUIRED_SECRET_ENV=(
  FLORESU_OAUTH_PRIVATE_KEY
  FLORESU_CLOUDFLARED_CREDENTIALS
  FLORESU_ALERTMANAGER_CONFIG
  FLORESU_CLOUDFLARED_INGRESS
...
# The ONLY ssh boundary that remains: read/write /opt/floresu/.deployed-sha (the
...
health_gate() {
  ...
  for ((i = 1; i <= 12; i++)); do
    ...
    raw="$(${COMPOSE_TUNNEL} ps --format json 2>/dev/null || true)"
...
main() {
  ...
  log "=== Floresu deploy -> context ${CONTEXT_NAME} (${SSH_TARGET}); dry-run=${DRY_RUN} ==="
