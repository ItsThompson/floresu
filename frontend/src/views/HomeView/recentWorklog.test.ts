...
describe("recentWorklog", () => {
  it("orders entries newest-date first", () => {
    ...
    expect(recentWorklog(entries, 5).map((entry) => entry.id)).toEqual([2, 3, 1]);
  });
  ...
  it("breaks same-date ties by newest id first", () => {
    ...
    expect(recentWorklog(entries, 5).map((entry) => entry.id)).toEqual([9, 8, 7]);
  });
  ...
  it("caps the result at the preview count, keeping the newest", () => {
    ...
    const preview = recentWorklog(entries, 5);
  ...
  it("does not mutate the input array", () => {
    ...
    recentWorklog(entries, 5);

    expect(entries.map((entry) => entry.id)).toEqual([1, 2]);
  });

  it("returns an empty array for no entries", () => {
    expect(recentWorklog([], 5)).toEqual([]);
  });
});
