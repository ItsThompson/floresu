 ...
 *
 * Crimson tint, and always with the message text: the failure is never signalled
 * by color alone.
 */
export function ConsentError({ message }: ConsentErrorProps) {
  return (
    <main className="bg-background text-foreground flex min-h-svh items-center justify-center p-6">
      <section
        role="alert"
        className="bg-destructive-tint border-destructive/40 text-destructive flex w-full max-w-[26rem] flex-col gap-4 rounded-xl border p-8"
      >
