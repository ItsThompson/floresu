"""Liveness and readiness endpoints for the MCP resource server.
...
- ``GET /readyz`` is readiness: every registered dependency check passes. Returns
  503 if any fails. The RS registers the AS-JWKS reachability check
  (:func:`floresu_mcp.keys.jwks_readiness_check`) here.

Readiness checks are injected, so this module stays dependency-free. Mirrors the
backend's ``floresu.core.health``; the RS ships as a separate image, so it owns
its own copy.
"""
