...
body: a machine ``code`` plus a human ``detail``, optionally a field-level
``fields`` map and a structural ``violations`` list that names offending IDs (a
stale-revision 409 says "re-read"; a DAG-cycle 422 names the nodes). This module
folds that body into a single :class:`ToolError` message so the agent sees a
structured, self-correctable error rather than a raw HTTP status.
...
(:mod:`floresu_mcp.tool_metrics`) can log them on ``tool_failed`` without
re-parsing.
"""
...
def _format_violations(violations: list[Any]) -> str:
    ...
    agent can locate and fix the exact records."""
    rendered: list[str] = []
