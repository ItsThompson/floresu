...
The MCP server owns the PRM; the backend AS owns the AS metadata. The PRM
advertises which authorization server protects this resource and the single scope
it supports, so a client that hits a 401 can discover the AS and run the OAuth
handshake. Every URL is built from **pinned** config (the ``resource`` is this
RS's public URL, the ``authorization_servers`` entry is the pinned AS issuer),
never a request host: the Site-URL gotcha applies to the RS as well.
...
from floresu_mcp.config import BEARER_METHOD_HEADER, PRM_PATH, SUPPORTED_SCOPES
...
def build_prm_document(*, resource: str, issuer: str) -> dict[str, Any]:
    ...
    OAuth endpoints from; ``scopes_supported`` advertises the single full scope.
    """
    return {
        "resource": resource,
