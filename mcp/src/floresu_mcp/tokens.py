...
and the granted ``client_id`` as the actor label. The token is **never** forwarded
downstream; the internal client exchanges it for the ``X-User-ID`` + ``X-Actor``
headers (see :mod:`floresu_mcp.client`). Crypto is delegated to joserfc; this
module owns only the claim contract.
"""
...
class VerifiedAgentToken:
    ...
    user_id: str  # the token ``sub``; the single user every call is scoped to
    client_id: str  # the granted client; becomes the X-Actor label downstream
    scope: str
