...
The backend AS holds the RSA private key and signs agent access tokens (RS256);
this RS verifies them against the AS's **public** JWKS. This module discovers the
JWKS URI from the AS metadata document (RFC 8414, built off the pinned issuer),
...
from floresu_mcp.health import CheckResult, ReadinessCheck
...
class RemoteKeyProvider:
    ...
    async def _ensure_loaded(self) -> KeySet:
        ...
        async with self._lock:
            ...
            assert self._key_set is not None  # narrows type after _reload_locked
            return self._key_set
...
def jwks_readiness_check(key_provider: KeyProvider) -> ReadinessCheck:
    ...
    async def check() -> CheckResult:
        ...
        except Exception as exc:  # any fetch/parse error is "not ready", not a crash
