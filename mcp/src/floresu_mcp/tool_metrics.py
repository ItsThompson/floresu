...
the operator can see agent activity and per-tool error rates. Kept on a dedicated
registry that :func:`floresu_mcp.metrics.instrument` exposes on ``/metrics``
alongside the private HTTP registry (names/labels follow a stable convention so
rules/dashboards drop in later).
...
follows ``__wrapped__``) produces an identical tool contract.
"""
...
from floresu_mcp.tool_errors import BackendToolError

_log = get_logger(SERVICE)

# Dedicated registry for MCP domain metrics, exposed alongside the private HTTP
# registry so a single process serves both without duplicate-timeseries errors.
