...
chain (:mod:`floresu_mcp.logging`), so every subsequent line: the tool layer's,
the internal-hop client's: carries the same ``request_id``. The internal client
forwards this id to the backend as ``X-Request-ID`` so one agent action is
traceable across the MCP -> backend hop.

Mirrors the backend's ``floresu.core.correlation.CorrelationMiddleware`` in shape
(pure-ASGI, mounted app-wide) with one deliberate difference: the RS is the
**origin** of an agent action, reached over the internet-facing agent surface, so
it always mints a fresh id and never honors an inbound ``X-Request-ID``. Being
app-wide (not folded into the bearer guard) means the non-guarded paths
(``/healthz``, ``/metrics``, the PRM document) are correlated too.
