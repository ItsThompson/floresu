...
- :class:`RsDeps` is the frozen façade over the injected dependencies the RS
  exposes on ``app.state`` (the JWKS key provider, the bearer verifier, the
  internal-API client, and the rate limiter). :func:`set_rs_deps` /
  :func:`get_rs_deps` store and read it under a single typed key.
...
from floresu_mcp.tokens import AgentTokenVerifier, VerifiedAgentToken
...
class RsDeps:
    ...
    Constructed once in :func:`floresu_mcp.app.create_rs_app` and stored via
    :func:`set_rs_deps`; the dataclass makes the seams a checked contract instead
    of untyped ``app.state`` writes."""
...
def get_request_agent(request: Request | None) -> VerifiedAgentToken | None:
    ...
    trusting an unchecked ``request.state`` value."""
    if request is None:
        return None
