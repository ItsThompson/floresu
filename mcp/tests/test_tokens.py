...
A token is accepted only if its RS256 signature verifies against the AS JWKS and
its ``iss``/``aud``/``exp`` claims hold; anything else resolves to ``None``
(rejected). Uses the real :class:`RemoteKeyProvider` with a faked JWKS fetch
(sociable): only the network is substituted, the joserfc crypto path runs for
real. On success ``sub`` becomes ``user_id`` and the granted ``client_id`` is
carried as the actor label.
"""
...
def _valid_claims() -> dict[str, object]:
    ...
    now = int(time.time())
    return {"iss": ISSUER, "aud": RESOURCE, "sub": "user-42", "iat": now, "exp": now + 3600}
...
async def test_valid_token_resolves_to_user_id_and_actor() -> None:
    key = new_key()
    verifier = _verifier([key])
    token = mint(key, sub="user-42", client_id="agent-7")

    principal = await verifier.verify(token)

    assert principal is not None
    assert principal.user_id == "user-42"
    assert principal.client_id == "agent-7"
    assert principal.scope == "floresu:full"
...
async def test_rotated_kid_is_accepted_after_a_refetch() -> None:
    ...
    key_provider._fetch_json = make_fetch(public_jwks(old_key, new_signing))
...
async def test_alg_none_token_is_rejected() -> None:
    # Algorithm substitution (unsigned token): the RS256 allowlist rejects
    # `alg=none` even with otherwise-valid claims.
...
async def test_hs256_rs256_algorithm_confusion_is_rejected() -> None:
    ...
    # the HMAC secret. The RS256 pin must reject it.
    key = new_key(kid="kid-1")
    verifier = _verifier([key])
